...
import {ReflectionDto, ReflectionSchema, toReflectionDto} from "./reflection.types";
...
export class ReflectionsRepository {
    ...
    async delete(reflectionId: string, userId: number): Promise<ReflectionDto | null> {
    ...
    async upsertMany(reflectionEntries: ReflectionDto[], userId: number) {
        ...
        await this.prisma.$transaction(async (tx) => {
                for (const entry of reflectionEntries) {
                    ...
                    !validation.success && results.entriesFailed.push({ entryId: entry.id, error: validation.error.message })
